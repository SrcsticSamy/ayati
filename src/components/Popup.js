    ...
    <div>
      <Modal id="myModal" centered show={show} onHide={handleClose} className="p-0" scrollable>
        ...
        <Modal.Body className="fw-bold text-wrap text-center lh-lg">
          <mark className="fs-4 d-block mb-1" dangerouslySetInnerHTML={{__html: ayahObj.ayahText}}>
          </mark>
          <Button variant={copied? 'success' : 'primary'} size="sm" onClick={()=>{navigator.clipboard.writeText(ayahObj.textToCopy); setCopied(true)}}>{copied? 'Copied ✓' : 'Copy'}</Button>
